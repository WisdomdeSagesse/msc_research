...
glm.fit <- glm(formula = STATUS ~ ., family = "binomial", data = train)
summary(glm.fit)

glm.probs <- predict(glm.fit, newdata = test, type = "response")

# calculating the test accuracy and error rate
glm.preds <- rep("defaulted", nrow(test))
glm.preds[glm.probs > 0.5] <- 'paid'
x_tab <- table(Predicted = glm.preds, True = test$STATUS)
confusionMatrix(x_tab)
```


```{r Logistic Regression with Cross Fold Validation}
library(boot)

glm.fit2 <- glm(STATUS ~ ., data = data, family = "binomial")

cv.error <- cv.glm(data, glm.fit2, K = 5)$delta[1]
...
```{r elastic regularization}
# from the results we can determine that at alpha = 0.0 we get the least test error rate

best.fit.model <- cv.glmnet(x_train, y_train, alpha = 0.0, type.measure = "class",
                            family = "binomial")
plot(best.fit.model)
...
```{r}
library(pROC)
roc(as.numeric(test$STATUS), as.numeric(best.fit.pred), plot = TRUE, legacy.axes = TRUE, 

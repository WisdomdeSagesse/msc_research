...
loan_data <- loan_data %>%
    select(ID, NAME, STATUS, LOAN_AMOUNT, SECTOR, COUNTRY,
...
```{r dealing with imbalanced classes}
summary(loan_data$STATUS)
kiva_loan_data <- upSample(loan_data[, -3], loan_data$STATUS, yname= "STATUS")
dim(kiva_loan_data)
```
